...
def HR_loader(batch_size):
    ...
    dataset = torchvision.datasets.ImageFolder('/home/bishshoy/ssd/DF2K/', transform=hr_transform)
    hr_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=12)
    
    return hr_loader


def save_resnet_weights(net, suffix, path='./resnet-weights/saved-model-', ):
    torch.save(net.state_dict(), path + str(suffix))
...
def test_resnet_rgb(net, hr_path='./test_HR_images/', sr_path='./test_SR_images/', save_file_prefix='', save_file_suffix=''):
    ...
    for i, data in enumerate(eval_loader):
        ...
        SR_RGB = np.array(np.round(np.clip(255 * SR[0].permute(1,2,0).cpu(), 0, 255)), dtype=np.uint8)
        misc.imsave(save_filename, SR_RGB)
...
def train_resnet():
    ...
    net.load_state_dict(torch.load('../SRResNet/weights/ckpt'), strict=False)
    test_resnet_rgb(net)
    ...
    while not end_flag:
        ...
        for i, data in enumerate(hr_loader):
            ...
            if global_step % save_freq == 0:
                save_resnet_weights(net, global_step, path='./resnet-weights-rgb/')
    ...
    while not end_flag:
        ...
        for i, data in enumerate(trainloader):
            ...
            if global_step % eval_freq == 0:
                ...
                # Eval images periodically
                test_resnet(G, hr_path='./test_HR/', sr_path='./test_SR_EaSRGAN/', save_file_suffix='-'+str(global_step), quiet=True)
...
def train_purana_gan():
    ...
    while not end_flag:
        ...
        for i, data in enumerate(trainloader):
            ...
            sys.stdout.write('\rIter:%i ' % global_step)
            sys.stdout.flush()
            ...
            if global_step % eval_freq == 0:
                ...
                # Eval images periodically
                # test_srgan(G)
                test_srgan(G, hr_path='./test_HR/', sr_path='./test_SR_SRGAN/', save_file_suffix='-' + str(global_step))
